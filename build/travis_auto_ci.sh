...
#    git remote add originx https://"${tokenx}"@github.com/mdj33/plugin.git >/dev/null 2>&1
#    git push --quiet --set-upstream originx $TRAVIS_BRANCH
    git push --force --quiet "https://${tokenx}@github.com/mdj33/plugin.git" origix:$TRAVIS_BRANCH
}
